  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        ...
        // ダンスカテゴリのIDを取得
        const lightCategory = categoriesData.contents.find(
          (category: { name: string }) => category.name === "照明"
        );
        ...
        const filteredArticles = data.contents.filter(
          (article: Article) => article.category.id === lightCategory.id
        );
